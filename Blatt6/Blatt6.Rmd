...
LGS mit Punkten -> Nicht Lösbar (nur approximierte Näherung)\newline
($f(t) = \sum_{k=0}^3(c_k\cdot e_k(t))$ und $e_0 = 1$, $e_1 = t$, $e_2 = t^2$ und $e_3 = t^3$ -> Lösbar) \newline
...
funktion  = c0*1+c1*x+c2*x*x
lines(x,funktion,col='red')
...
funktion  = c0*1+c1*x+c2*x*x+c3*x*x*x
lines(x,funktion)
```
